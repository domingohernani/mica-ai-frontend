...
import TIME_SLOTS from "../../constants/interview-schedule-timeslots.constant";
import INTERVIEW_DURATIONS from "../../constants/interview-schedule-duration";
import {
  type Application,
...
const ScoreRing = ({ score }: { score: number }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-2">
      ...
      <span
        className={cn("text-xs font-medium px-2.5 py-1 rounded-full", labelCls)}
      >
        {label}
      </span>
    </div>
      ...
      <div className="grid grid-cols-12 gap-3">
        ...
        <Card className="col-span-12 md:col-span-9">
          <CardHeader>
            ...
            <CardDescription>
              AI-generated assessment of candidate fit
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="col-span-12 md:col-span-7">
          <CardHeader>
            ...
            <CardDescription>
              ...
              <Badge className="ml-2">{matchedRequirements.length}</Badge>
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="col-span-12 md:col-span-5">
          <CardHeader>
            ...
            <CardDescription>
              ...
              <Badge className="ml-2 font-medium normal-case">
...
const HiringScheduleInterviewPage = () => {
  ...
  const scheduleMutation = useMutation({
    mutationFn: async () => {
      const baseDate = DateTime.fromJSDate(selectedDate); // e.g., '2026-06-02'
      const timePart = DateTime.fromFormat(selectedTime, "hh:mm a"); // e.g., 16:00

      const combinedDateTime = baseDate.set({
        hour: timePart.hour,
        minute: timePart.minute,
        second: 0,
        millisecond: 0,
      });
      const utcIsoString = combinedDateTime.toUTC().toISO();

      const { data } = await api.post(
        `/jobs/${jobId}/applications/${applicationId}/schedule-interview`,
        {
          scheduledDate: utcIsoString,
  ...
  const handleBackBtn = () => {
    const talentAcquisitionSection = SIDEBAR_ITEMS.find(
      (sidebar) => sidebar.label === "Talent Acquisition",
    );
    const candidatesPage = talentAcquisitionSection?.items.find(
      (page) => page.title === "Candidates",
    );
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <Card className="col-span-12 md:col-span-8">
          <CardHeader>
            ...
            <CardDescription>
              Current role, experience, and expectations
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        {applicant.applicantEvaluation && (
          <GeminiEvaluationSection
            evaluation={applicant.applicantEvaluation.evaluation}
          />
        )}
        {/* Interview Date */}
        <Card className="col-span-12 md:col-span-5">
          <CardHeader>
            ...
            <CardDescription>
              Pick a date for the interview session
            </CardDescription>
          </CardHeader>
        ...
        <Card className="flex flex-col col-span-12 md:col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              Set the length and confirm the booking
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col flex-1 gap-4 px-4 pb-4">
            ...
            <div
              className={cn(
                "space-y-1.5 flex-1 transition-opacity",
                canSchedule ? "opacity-100" : "opacity-40",
              )}
            >
            ...
            <div className="flex items-start gap-2 rounded-md bg-muted p-2.5">
              ...
              <p className="text-sm leading-relaxed text-muted-foreground">
                {applicant.firstName} receives a calendar invite and interview
