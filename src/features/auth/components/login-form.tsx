...
const LoginForm = ({ className, ...props }: React.ComponentProps<"div">) => {
  ...
  const handleSocialButton = async (authService: string) => {
    try {
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      {isAuthenticated ? "Is authenticated!" : "Not yet"}
      <form onSubmit={handleContinueButton}>
        <FieldGroup>
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            <h1 className="text-xl font-bold">Welcome to Mica AI</h1>
            <FieldDescription>
              Smart interviews that elevate your team’s decisions and spotlight
              the talent that truly stands out.
            </FieldDescription>
          </div>
          <Field>
          ...
          <Field className="flex flex-col gap-4">
            ...
            <Button
              variant="outline"
              type="button"
              onClick={() => handleSocialButton("google-oauth2")}
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path
